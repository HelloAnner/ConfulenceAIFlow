class NotificationService {
  ...
  // 发送通知
  async sendNotification(type, webhookUrl, content, title = 'Confluence AI 分析结果') {
    try {
  ...
  formatForWechat(content, title) {
    // 如果content是对象（AI分析结果），则格式化为markdown
    let markdownContent;
    if (typeof content === 'object') {
      markdownContent = this.formatAnalysisResultToMarkdown(content);
    } else {
