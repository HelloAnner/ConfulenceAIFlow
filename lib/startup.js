...
export async function initializeServices() {
  try {
    ...
    if (process.env.SCHEDULER_ENABLED === 'true') {
      ...
      console.log(`⚡ 其中 ${activeConfigs.length} 个配置具有定时任务`);
    }
    ...
    return {
      ...
      activeJobs: process.env.SCHEDULER_ENABLED === 'true' ? configService.getActiveConfigs().length : 0
    };
  ...
  try {
    const status = {
      ...
      services: {
        configService: {
          status: 'running',
          configsCount: configService.getAllConfigs().length
        },
        schedulerService: {
          status: schedulerService.isRunning ? 'running' : 'stopped',
          jobsCount: schedulerService.isRunning ? schedulerService.getAllJobsStatus().totalJobs : 0
        }
      },
      environment: {
